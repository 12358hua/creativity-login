...
const SwimmerBeach = () => {
  ...
  // --- draw one animated swimmer ---
  const drawSwimmer = (rc, x, y, fi) => {
    const phase = (fi / FRAME_COUNT) * Math.PI * 2
    const armSwing = Math.sin(phase) * 5
    const legSwing = Math.cos(phase) * 4

    // head
    rc.circle(x, y - 8, 4.5, {
      fill: '#ffcc99', stroke: '#8B4513', fillWeight: 1.5, roughness: 0.3,
    })
    // torso
    rc.linearPath([[x, y - 4], [x, y + 10]], {
      stroke: '#8B4513', strokeWidth: 1.5,
    })
    // left arm
    rc.linearPath([[x, y + 1], [x - 9, y - 3 + armSwing]], {
      stroke: '#8B4513', strokeWidth: 1.3,
    })
    // right arm
    rc.linearPath([[x, y + 1], [x + 9, y - 3 - armSwing]], {
      stroke: '#8B4513', strokeWidth: 1.3,
    ...
    // left leg
    rc.linearPath([[x, y + 10], [x - 6, y + 18 + legSwing]], {
      stroke: '#8B4513', strokeWidth: 1.3,
    })
    // right leg
    rc.linearPath([[x, y + 10], [x + 6, y + 18 - legSwing]], {
    ...
    // water splashes
    rc.linearPath([[x - 7, y - 8], [x - 11, y - 10], [x - 8, y - 6]], {
      stroke: 'lightskyblue', strokeWidth: 1, roughness: 1.5,
    })
    rc.linearPath([[x + 7, y - 8], [x + 11, y - 10], [x + 8, y - 6]], {
      stroke: 'lightskyblue', strokeWidth: 1, roughness: 1.5,
    })
  ...
  const draw = (ctx, rc, fi) => {
    ...
    // --- two swimming people (left side, away from boat) ---
    drawSwimmer(rc, 100, 150, fi)
    drawSwimmer(rc, 170, 160, fi)

    // icons (drawn last so they sit on top)
